...
*   **File cài đặt đã tạo:** `C:\Projects\central_logger\installer_build\CentralLoggerSetup.exe`
...
```text
c:\Projects\central_logger\installer_build\
├── config\
...
> Để bảo toàn 100% độ sắc nét và tính nguyên bản của thương hiệu, các tệp ảnh thương hiệu trên (`app_icon.ico`, `logo.png`, `window_icon.png`) được sinh tự động trực tiếp từ tệp thiết kế vector gốc **`resources/icons/brand_4m_technologies_blue.svg`** bằng một công cụ biên dịch Qt C++ chuyên dụng (`convert_tool`).
...
Để hỗ trợ bạn đóng gói nhanh chóng sau khi chỉnh sửa code hoặc build Release phiên bản mới trong Qt Creator, tôi đã tạo một script PowerShell tự động hóa toàn bộ quy trình: [**`build_installer.ps1`**](file:///c:/Projects/central_logger/build_installer.ps1).
...
2.  Mở PowerShell tại thư mục dự án `C:\Projects\central_logger`.
3.  Chạy lệnh sau:
    ```powershell
    .\build_installer.ps1
    ```
4.  Đợi khoảng 15-20 giây, bộ cài đặt mới `CentralLoggerSetup.exe` sẽ sẵn sàng trong thư mục `installer_build/` để bạn gửi cho khách hàng hoặc test triển khai.
